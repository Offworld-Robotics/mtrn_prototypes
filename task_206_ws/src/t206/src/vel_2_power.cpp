...
//    std_msgs::Float64 angle = 0.0;
    float xValue = msg->linear.x; //Move forward or backward
    float yValue = msg->linear.y; //Turning
    std_msgs::Float64 powPercent;
    powPercent.data = 0.0;
    float angle = 0.0;

    if (yValue == 0) { // Without turning
        powPercent.data = xValue / this->maxSpeed;
        for (int i = 0; i < 4; i++) {
        ...
        powPercent.data = abs(angle) / 90.0;
